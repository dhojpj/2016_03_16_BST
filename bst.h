...
enum BST_TRAVERSAL_TYPES {IN_ORDER, PRE_ORDER, POST_ORDER}; // call this?
enum BST_ERROR_TYPES {EMPTY}; // when to "call" this?
...
void bst<T>::insert(const T &d, size_t c)
{
    ...
    else
    {
        node<T> *temp = root;
        while(notDone)
            if(d == *temp) // from the == operator in node
            {
                (*temp) += c; // increment its count
                notDone = false; // stop the loop
            }
            else
            {
                // int data type = direction array contains RIGHT, LEFT
                // if data is less than root's data, go left
                // if greater, go right
                ...
                {
                    temp->childPtr(child) = new node<T>(d,c); // insert the node
                    notDone = false; // stops the while loop
                }
                else
                    temp = temp->childPtr(child); // go deeper, and go back to top of while loop
            }
    }
...
{
    return !r ? NULL :
                r->childPtr(LEFT)->getData() == d
                ||
                r->childPtr(RIGHT)->getData() == d
                ?
                r
...
// this was written here because it's called more than once (twice in fact)
template<typename T>
